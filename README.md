...
* *User Auth Service* (Spring Boot + Java) - Port 8082
* *Quiz Service* (Go) - Port 8083
* *Class Assignment Service* (Spring Boot + Java) - Port 8084
* *Analytics & Statistic Service* (FastAPI + Python) - Port 8085 (proxy nội bộ 8084)
...
* *Docker Compose* - Orchestrator cho toàn bộ stack
...
### Run toàn bộ hệ thống bằng Docker Compose (recommended)

```bash
git clone <YOUR_REPO_URL> SE2025-17.1
cd SE2025-17.1

# Tạo file .env theo hướng dẫn trong DEPLOY.md
...
- Frontend (UI): `http://localhost` (qua Nginx, port 80)
- API Gateway: `http://localhost/api/v1/...`
- Các service backend truy cập nội bộ qua Docker network (xem chi tiết trong `docker-compose.prod.yml`).
...
* *API Integration:* Tất cả API backend được gọi thông qua Nginx với prefix `/api/v1/...`. Ví dụ cấu hình môi trường local:

```env
NEXT_PUBLIC_API_URL=http://localhost/api/v1
NEXT_PUBLIC_AUTH_API_URL=http://localhost/api/v1
NEXT_PUBLIC_QUIZ_API_URL=http://localhost/api/v1
NEXT_PUBLIC_NOTIFICATION_API_URL=http://localhost/api/v1
```
...
* **Sơ đồ kiến trúc:** 
  * `docs/architecture-diagram.drawio` (mở bằng draw.io)
  * Use Case Diagram: `docs/use-case-diagram.png` (hoặc `.jpg`/`.svg`)
