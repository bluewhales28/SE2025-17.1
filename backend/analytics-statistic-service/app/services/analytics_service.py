...
class AnalyticsService:
    ...
    async def analyze_student_progress(
        self, student_id: int, quiz_results: List[Dict[str, Any]]
    ) -> Dict[str, Any]:
        """
        ...
        # Determine trend
        if len(df) >= 2:
            # Use half of data or minimum 5 items for comparison
            n = len(df)
            split_point = max(1, n // 2)
            
            # Compare first half with second half
            early_avg = df["score"].head(split_point).mean()
            recent_avg = df["score"].tail(n - split_point).mean()
    ...
    async def calculate_class_metrics(
        self, class_id: int, student_results: List[Dict[str, Any]]
    ) -> Dict[str, Any]:
        """
        ...
        completion_rate = (
            (df["student_id"].nunique() / unique_students * 100)
    ...
    async def analyze_question_performance(
        self, question_id: int, question_answers: List[Dict[str, Any]]
    ) -> Dict[str, Any]:
        """
        ...
        difficulty_index = (
            correct_attempts / total_attempts if total_attempts > 0 else 0.5
        )
        ...
        if len(df) >= 10 and "total_score" in df.columns:
            ...
            discrimination_index = (
                ((high_correct - low_correct) / top_27_percent)
                if top_27_percent > 0
                else 0.0
            )
        else:
        ...
        return {
            ...
            "average_time_seconds": (
                float(df["time_spent"].mean()) if "time_spent" in df.columns else 0.0
            ),
        }
    ...
    async def analyze_by_topic(
        ...
        if "question_id" in results_df.columns:
            merged_df = results_df.merge(questions_df, on="question_id", how="left")
            ...
            if "topic" in merged_df.columns:
                ...
                for topic in topic_analysis.index:
                    topic_dict[topic] = {
                        "correct_count": int(topic_analysis.loc[topic, ("is_correct", "sum")]),
                        "total_count": int(topic_analysis.loc[topic, ("is_correct", "count")]),
                        "accuracy_rate": float(topic_analysis.loc[topic, ("is_correct", "mean")] * 100),
                    }
    ...
    async def calculate_score_distribution(
        self, quiz_results: List[Dict[str, Any]], bins: int = 10
    ) -> Dict[str, int]:
        """
