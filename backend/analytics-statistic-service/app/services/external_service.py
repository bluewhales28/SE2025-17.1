...
class QuizServiceClient(BaseServiceClient):
    ...
    async def get_quiz(self, quiz_id: int, token: Optional[str] = None) -> Optional[Dict[str, Any]]:
        """Get quiz details"""
        headers = {"Authorization": f"Bearer {token}"} if token else {}
        return await self._request("GET", f"/api/v1/quiz/{quiz_id}", headers=headers)
    
    async def get_quiz_questions(self, quiz_id: int, token: Optional[str] = None) -> Optional[List[Dict[str, Any]]]:
        """Get quiz questions"""
        headers = {"Authorization": f"Bearer {token}"} if token else {}
        response = await self._request("GET", f"/api/v1/quiz/{quiz_id}/questions", headers=headers)
        return response.get("questions", []) if response else []
    
    async def get_quiz_results(self, quiz_id: int, token: Optional[str] = None) -> Optional[List[Dict[str, Any]]]:
        """Get all results for a quiz"""
        headers = {"Authorization": f"Bearer {token}"} if token else {}
        response = await self._request("GET", f"/api/v1/quiz/{quiz_id}/results", headers=headers)
    ...
    async def get_student_quiz_attempts(
        ...
        response = await self._request(
            "GET",
            f"/api/v1/student/{student_id}/attempts",
            headers=headers
        )
    ...
    async def get_question_details(
        ...
        headers = {"Authorization": f"Bearer {token}"} if token else {}
        return await self._request("GET", f"/api/v1/question/{question_id}", headers=headers)
...
class ClassServiceClient(BaseServiceClient):
    ...
    async def get_class(self, class_id: int, token: Optional[str] = None) -> Optional[Dict[str, Any]]:
        """Get class details"""
        headers = {"Authorization": f"Bearer {token}"} if token else {}
    ...
    async def get_class_students(
        ...
        response = await self._request(
            "GET",
            f"/api/v1/class/{class_id}/students",
            headers=headers
        )
        return response.get("students", []) if response else []
    
    async def get_student(self, student_id: int, token: Optional[str] = None) -> Optional[Dict[str, Any]]:
        """Get student details"""
        headers = {"Authorization": f"Bearer {token}"} if token else {}
        return await self._request("GET", f"/api/v1/student/{student_id}", headers=headers)
    ...
    async def get_class_quizzes(
        ...
        response = await self._request(
            "GET",
            f"/api/v1/class/{class_id}/quizzes",
            headers=headers
        )
...
class NotificationServiceClient(BaseServiceClient):
    ...
    async def send_certificate_email(
        ...
        response = await self._request(
            "POST",
            "/api/v1/notification/certificate",
            headers=headers,
            json=payload
        )
        return response is not None
