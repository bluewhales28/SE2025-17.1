...
class FraudDetectionService:
    ...
    async def detect_similar_submissions(
        self, quiz_id: int, submissions: List[Dict[str, Any]]
    ) -> List[Dict[str, Any]]:
        """
        ...
        for i in range(len(submissions)):
            for j in range(i + 1, len(submissions)):
                ...
                similarity = self._calculate_submission_similarity(
                    submission1.get("answers", []), submission2.get("answers", [])
                )
    ...
    async def detect_abnormal_speed(
        self, quiz_id: int, submission: Dict[str, Any], average_time: float
    ...
    async def detect_pattern_anomaly(
        self, quiz_id: int, submission: Dict[str, Any]
    ) -> Optional[Dict[str, Any]]:
        """
        ...
        selected_options = [
            a.get("selected_option") for a in answers if "selected_option" in a
        ]
        ...
        if len(selected_options) >= 4:
            ...
            for i in range(len(selected_options) - pattern_length + 1):
                ...
                for j in range(
                    i + pattern_length, len(selected_options) - pattern_length + 1
                ):
                    if selected_options[j : j + pattern_length] == pattern:
                        pattern_count += 1

            pattern_ratio = (
                pattern_count / (len(selected_options) // pattern_length)
                if len(selected_options) >= pattern_length
                else 0
            )
    ...
    def _calculate_submission_similarity(
        self, answers1: List[Dict[str, Any]], answers2: List[Dict[str, Any]]
    ) -> float:
        """
    ...
    async def resolve_alert(
        self, alert_id: int, resolved_by: int, resolution_note: Optional[str] = None
    ) -> AlertEvent:
        """
