...
class PDFService:
    ...
    async def generate_report_pdf(
        ...
        if report_type == "quiz":
            story.append(
                Paragraph(
                    f"Quiz Report: {data.get('quiz_title', 'Unknown')}", title_style
                )
            )
        elif report_type == "student":
            story.append(
                Paragraph(
                    f"Student Progress Report: {data.get('student_name', 'Unknown')}",
                    title_style,
                )
            )
            ...
            story.append(
                Paragraph(
                    f"Class Statistics: {data.get('class_name', 'Unknown')}",
                    title_style,
                )
            )
        ...
        story.append(
            Paragraph(
                f"Generated on: {datetime.now().strftime('%Y-%m-%d %H:%M')}", date_style
            )
        )
    ...
    def _add_student_statistics(self, data: Dict[str, Any]) -> List:
        ...
        table_data = [
            ...
            [
                "Total Time Spent",
                f"{data.get('total_time_spent_minutes', 0):.2f} minutes",
            ],
        ]
    ...
    async def generate_certificate(
        self, student_name: str, certificate_data: Dict[str, Any]
    ) -> str:
        """
        ...
        description = certificate_data.get(
            "description", "Has successfully completed the quiz"
        )
        ...
        story.append(
            Paragraph(f"Certificate Number: {certificate_number}", footer_style)
        )
