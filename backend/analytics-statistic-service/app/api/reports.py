...
from app.schemas.report import (ClassStatisticsResponse,
...
async def get_quiz_report(
    ...
    result = await cache_service.get_or_compute(
        cache_key, compute_quiz_report, ttl=1800  # 30 minutes
    )
...
async def get_student_progress(
    ...
    async def compute_student_progress():
        ...
        return {
            ...
            "completion_rate": (
                (len(completed) / len(attempts) * 100) if attempts else 0
            ),
    ...
    result = await cache_service.get_or_compute(
        cache_key, compute_student_progress, ttl=900  # 15 minutes
    )
...
async def get_class_statistics(
    ...
    async def compute_class_statistics():
        ...
        analytics = AnalyticsService(db)
        class_metrics = await analytics.calculate_class_metrics(
            class_id, student_results
        )
    ...
    result = await cache_service.get_or_compute(
        cache_key, compute_class_statistics, ttl=1800  # 30 minutes
    )
...
async def get_question_analysis(
    ...
    cache_key = cache_service.generate_cache_key(
        "question_analysis", question_id=question_id
    )
    ...
    async def compute_question_analysis():
        ...
        analytics = AnalyticsService(db)
        analysis = await analytics.analyze_question_performance(
    ...
    result = await cache_service.get_or_compute(
        cache_key, compute_question_analysis, ttl=3600  # 60 minutes
    )
